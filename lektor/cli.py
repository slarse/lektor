...
def buildflag_deprecated(ctx, param, value):
    if value:
        warnings.warn("use --extra-flag instead of --build-flag", DeprecationWarning)
    return value
...
def plugins_add_cmd(ctx, name):
    ...
    try:
        ...
        click.echo(
            "Package %s (%s) was added to the project" % (info["name"], info["version"])
        )
...
def plugins_remove_cmd(ctx, name):
    ...
    try:
        ...
        if old_info is None:
            ...
            click.echo(
                "Removed package %s (%s)" % (old_info["name"], old_info["version"])
            )
